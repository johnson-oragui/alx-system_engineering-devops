...
def count_words(subreddit, word_list, instances={}, after="", count=0):
    ...
    for c in results.get("children"):
        # Extract the title of the current post, convert to lowercase, and split into words
        title = c.get("data").get("title").lower().split()
    
        # Iterate through each word in the word_list
        for word in word_list:
            # Check if the lowercase version of the current word is present in the title
            if word.lower() in title:
                # Count the occurrences of the current word in the title
                times = len([t for t in title if t == word.lower()])
            
                # Check if an entry for the word exists in the instances dictionary
                if instances.get(word) is None:
                    # If not, initialize an entry with the times value
                    instances[word] = times
                else:
                    # If entry exists, add the times value to the existing count
                    instances[word] += times
    ...
    if after is None:
        ...
        # Sort the instances dictionary by values in descending order, then by keys
